...
#pragma NDIS_INIT_FUNCTION(ntfwkmRegisterDevice)


_IRQL_requires_max_(PASSIVE_LEVEL)
NDIS_STATUS
ntfwkmRegisterDevice(
    VOID
    )
{
    ...
    DEBUGP(DL_TRACE, "==>ntfwkmRegisterDevice\n");
   
    
    NdisZeroMemory(DispatchTable, (IRP_MJ_MAXIMUM_FUNCTION+1) * sizeof(PDRIVER_DISPATCH));
    
    DispatchTable[IRP_MJ_CREATE] = ntfwkmDispatch;
    DispatchTable[IRP_MJ_CLEANUP] = ntfwkmDispatch;
    DispatchTable[IRP_MJ_CLOSE] = ntfwkmDispatch;
    DispatchTable[IRP_MJ_DEVICE_CONTROL] = ntfwkmDeviceIoControl;
    ...
    DEBUGP(DL_TRACE, "<==ntfwkmRegisterDevice: %x\n", Status);
